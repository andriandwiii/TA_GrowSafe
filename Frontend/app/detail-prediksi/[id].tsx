...
import { View, Text, StyleSheet, StatusBar, TouchableOpacity, ScrollView, Dimensions, Alert } from 'react-native';
...
const DetailPrediksiScreen = () => {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      if (!id || !kumbungAktif) return;
      try {
        setIsLoading(true);

        // Fetch detail prediksi
        const prediksiRes = await apiClient.get(`/history/prediksi/detail/${id}`);
        const result = prediksiRes.data;

        // Fetch profil kumbung untuk dapatkan kapasitas
        let kapasitasBaglog = 100; // default fallback
        try {
          ...
          console.log('Gagal ambil data kumbung', e);
        }
        ...
        console.error('Error memuat detail prediksi:', error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    Alert.alert(
      ...
      [
        { text: "Batal", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await apiClient.delete(`/history/prediksi/${id}`);
              router.back();
            } catch (error) {
              console.error("Gagal menghapus data:", error);
              Alert.alert("Error", "Gagal menghapus riwayat prediksi.");
            }
          }
        }
      ]
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {isLoading || !predictionData ? (
        <CustomLoading fullScreen message="Memuat detail..." />
      ) : (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>

          {/* Date Label */}
          <Animated.View entering={FadeInDown.delay(50).springify()} style={{ alignItems: 'center', marginBottom: 16 }}>
            <View style={{ backgroundColor: '#FFFFFF', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: '#E2E8F0' }}>
              <Text style={{ fontFamily: 'Poppins-Medium', fontSize: 12, color: '#64748B' }}>{predictionData.tanggal}</Text>
            </View>
          </Animated.View>
          ...
          <Animated.View entering={FadeInDown.delay(100).springify()} style={styles.mainCard}>
            <View style={styles.mainCardHeader}>
              ...
              <Text style={styles.mainCardTitle}>ESTIMASI POTENSI PANEN</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  bgDecorTop: { position: 'absolute', top: 0, left: 0, right: 0, height: 220, backgroundColor: '#E8F5E9', borderBottomLeftRadius: 40, borderBottomRightRadius: 40, opacity: 0.5 },
  ...
  mainCard: { backgroundColor: '#FFFFFF', borderRadius: 24, padding: 24, marginBottom: 16, shadowColor: '#94A3B8', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.1, shadowRadius: 16, elevation: 4, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(46, 125, 50, 0.1)' },
  mainCardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 24, alignSelf: 'flex-start' },
  mainCardTitle: { fontFamily: 'Poppins-SemiBold', fontSize: 14, color: '#1E293B', letterSpacing: 0.5 },
  yieldContainer: { alignItems: 'center', marginBottom: 20 },
  yieldNumber: { fontFamily: 'Poppins-Bold', fontSize: 64, color: '#2E7D32', includeFontPadding: false, lineHeight: 70 },
  yieldUnit: { fontFamily: 'Poppins-Medium', fontSize: 16, color: '#64748B', marginTop: 4 },
  yieldSubtextContainer: { backgroundColor: '#F8FAFC', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12, width: '100%', alignItems: 'center', borderWidth: 1, borderColor: '#E2E8F0' },
  yieldSubtext: { fontFamily: 'Poppins-Medium', fontSize: 12, color: '#475569', textAlign: 'center' },
  gridContainer: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', marginBottom: 16 },
  gridCard: { width: (width - 40 - 16) / 2, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 20, marginBottom: 16, justifyContent: 'center', alignItems: 'center', shadowColor: '#94A3B8', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.05, shadowRadius: 12, elevation: 2 },
  gridLabel: { fontFamily: 'Poppins-SemiBold', fontSize: 11, color: '#64748B', marginBottom: 8, letterSpacing: 0.5, textAlign: 'center' },
  gridValue: { fontFamily: 'Poppins-Bold', fontSize: 22, color: '#1E293B', textAlign: 'center' },
  categoryBadge: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
  categoryText: { fontFamily: 'Poppins-Bold', fontSize: 18, color: '#059669' },
  formulaCard: { backgroundColor: '#F1F5F9', borderRadius: 16, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#E2E8F0' },
  formulaText: { fontFamily: 'Poppins-Medium', fontSize: 12, color: '#475569', lineHeight: 22, textAlign: 'center' },
  recommendationCard: { flexDirection: 'row', backgroundColor: '#ECFDF5', borderRadius: 20, padding: 20, borderWidth: 1, borderColor: '#A7F3D0', alignItems: 'flex-start' },
  recommendationText: { flex: 1, fontFamily: 'Poppins-Medium', fontSize: 14, color: '#065F46', marginLeft: 12, lineHeight: 22 },
